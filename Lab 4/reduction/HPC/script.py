...
runs = 10
procs = 5 #0,4,8,12,16
prob =  8 #16,32,64,..,4096
problem_size = [x for x in range(1, 9)]
...
with open('data.txt','r') as file:
    ...
    for line in lines:
        ...
        words[0] = int(round(math.log(words[0],10)))-1
...
for i in range(procs):
    ...
    plt.legend()
plt.savefig('mean_execution_time_q2_multiplication.png')
plt.figure()
plt.grid()
plt.xlabel('logN')
plt.ylabel('Speedup')
for i in range(procs):
    ...
    plt.legend()
plt.savefig('speedup_q2_multiplication.png')
plt.figure()
plt.grid()
plt.xlabel('logN')
...
for i in range(procs):
    ...
    plt.legend()
plt.savefig('efficiency_q2_multiplication.png')
